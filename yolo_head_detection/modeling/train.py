...
def main():
    ...
    try:
        ...
        with mlflow.start_run(run_name=TRAINER.run) as run:
            ...
            model = YOLO(model=MODELS_DIR / TRAINER.model, verbose=True)
            ...
            model_input = np.array(
                Image.open(REPORTS_DIR / "figures" / "mov_019_022253.jpeg")
                .convert("RGB")
                .resize((640, 640))
            )
            ...
            model_signature = mlflow.models.infer_signature(
                model_input=batch_input, model_output=batch_output
            )
